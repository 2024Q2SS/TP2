...
def draw_grid_2d(cells, size, frame_number):
    ...
    # Add the frame counter in the top-left corner
    font = ImageFont.load_default()  # You can load a different font if needed
    draw.text((10, 10), f"Frame {frame_number}", fill="black", font=font)

    return img

# Function to draw the grid in 3D based on the current state
def draw_grid_3d(cells, size, frame_number):
    fig = plt.figure()
...
for i, file in enumerate(files):
    with open(os.path.join(output_folder, file), "r") as f:
        cells = json.load(f)
    if dimensions == 2:
        img = draw_grid_2d(cells, canvas_size, i + 1)
    elif dimensions == 3:
        img = draw_grid_3d(cells, canvas_size, i + 1)
    images.append(img)
