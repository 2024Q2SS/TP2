...
public class Board {
    ...
    private Map<Cell,Set<Cell>> neighbourMap = new HashMap<>();
    ...
    public Boolean inSpawnArea(Integer x, Integer y,Integer spawnArea){
        if((x >= 0.5*size-spawnArea && y >= 0.5*size-spawnArea) && (x <= 0.5*size+spawnArea && y <= 0.5*size+spawnArea))
            return true;
        return false;
    }
    ...
    public Boolean finalState(){
       for(int x = 0; x < size; x+=size-1){
            for(int y = 0; y < size; y++){
                if(cellMap.get(new Coordinates(x,y)).getState() == State.ALIVE)
                    return true;
            }
        }
